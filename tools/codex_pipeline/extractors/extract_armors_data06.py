...
def diff_armors_by_name(old_path: Path, new_path: Path) -> list[str]:
    try:
    ...
    for item_id in all_ids:
        ...
        if old_item is None:
            ...
            lines.append(f"Item {item_id}: {name}")
            lines.append("  + added")
            lines.append("")
            continue
        ...
        if diffs:
            ...
            lines.append(f"Item {item_id}: {name_display}")
            lines.extend(diffs)
            lines.append("")
...
def main(argv=None):
    ...
    if backup_info:
        ...
        diff_lines = diff_armors_by_name(backup_path, out_path)
        if diff_lines:
            if diff_out_path is not None:
