...
class SiteSmokeTests(unittest.TestCase):
    ...
    def test_node_runner_includes_build_planner_flow(self):
        ...
        self.assertIn('data-element="Dark"', runner)
        self.assertIn('data-element="dark"', runner)
        self.assertIn("Dark 1.3x", runner)
        self.assertIn("Best set vs. Dark", runner)
        self.assertIn("Monster recommendation tooltip overflowed mobile viewport", runner)
