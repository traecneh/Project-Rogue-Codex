      (function () {
        ...
        const LEGACY_RARITY_INDEX_MAP = [0, 1, 1, 2, 2, 3, 4];
        const migrateRarityIndex = (value, sourceVersion) => {
          const index = Math.max(0, Math.trunc(toNumber(value)));
          if (toNumber(sourceVersion) >= BUILD_STATE_VERSION) {
            return Math.min(index, RARITY_TIERS.length - 1);
          }
          return LEGACY_RARITY_INDEX_MAP[index] ?? RARITY_TIERS.length - 1;
        };
        ...
        const decodeStatePayload = (raw) => {
          ...
          if (Array.isArray(parsed.slots)) {
            return {
              ...parsed,
              slots: parsed.slots.map((entry) => ({
                ...entry,
                rarityIndex: migrateRarityIndex(entry?.rarityIndex, parsed.v),
              })),
            };
          }
          return null;
        ...
        const updateSlotPerkUI = (slot, clearOnly = false) => {
          ...
          if (clearOnly || !data || data.rarityIndex < 2) {
            if (row) row.style.display = "none";
